...
<body id="page-top">
	...
	<div class="container">
		...
		<div class="page-header ">
	  			...
	  			<input type = "text" name = "title" id = "dealer_news_title" style="border:none;width:100%" value = "${pageMaker.title}" readOnly/>
	  			</div>
	  			<span class="btcinfoland_writer">작성자 : ${pageMaker.writer}</span>
				<span class="btcinfoland_writeDate">작성일 : 
				<fmt:formatDate pattern = "yyyy-MM-dd HH:mm" value = "${pageMaker.regi_date}" />
				</span>
				...
				<div class="board_nav">
					<span id = "previous_news_btn" class="glyphicon glyphicon-chevron-left" data-toggle="tooltip" title="이전글"></span>
					<span id = "next_news_btn" class="glyphicon glyphicon-chevron-right" data-toggle="tooltip" title="다음글"></span>
				</div>
			</div>
		
			<div class="container">
				<p class="board_content">
					<textarea name = "content" style="border:none;width:100%" 
					id = "dealer_news_content" readOnly/>${pageMaker.content}</textarea>
				</p>
			</div>
		...
		<div class="container">
			<div class="btn_area">
				<button type="button" class="btn btn-default" id = "reply_list_btn">댓글보기</button>
			</div>	
		</div>
		...
		<div class="container">		
			...
			<button type="button" class="btn btn-primary" id = "write_reply_btn">
				<span class="labe label-primary">답글</span>
			</button>
		</div>
		...
		<form id = "form_previous_news" action = "/getPreviousNews"> 
			<input type = "hidden" name = "dealer_news_num" 
			value = "${pageMaker.dealer_news_num}" />
			<input type = "hidden" name = "currentPage" 
			value = "${pageMaker.currentPage}"/>
		</form>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="form-group">
					...
					name = "content" id="write_reply_content" placeholder="내용을 입력하세요"/>
					<br>
		...
		$(document).ready(function(){
			$('#modify_btn').on('click', function(){
				...
				if ($('textarea').prop('readOnly') == true){
					...
					$('textarea').attr("style","border-style: solid; width: 100%");
					/* 읽기 전용이 풀린다 */
					...
					$('#form_modify input[name="content"]').val($('#dealer_news_content').val());
					$('#form_modify').submit();
				}
			});
	...
	<script>
		function getDealerNewsReplyList(){
			$.ajax({
				...
				data : { 'dealer_news_num' : '${pageMaker.dealer_news_num}',
		...
		function listReply(data){
			...
			for (var i = 0; i < data.length; i++){
				...
				*/
				...
				$('#reply_list').append('<tr id = "' + data[i].reply_num + '">');
				
				$('#'+ data[i].reply_num).append(data[i].reply_num);
				...
				$('#' + data[i].reply_num).append(data[i].content);
				
				$('#' + data[i].reply_num).append('<a href = "#" id ="write_reply">답글</a>');
				$('#' + data[i].reply_num).append('<a href = "#" id ="delete_reply">삭제</a>');
				$('#' + data[i].reply_num).append((new Date(data[i].regi_date).getDate()) + "일" + (new Date(data[i].regi_date).getHours())+"시" + (new Date(data[i].regi_date).getMinutes())+"분");
				$('#' + data[i].reply_num).append('(<a href = "#" class = "reply_writer">' + data[i].writer + '</a>님)');


				$('#' + data[i].reply_num).append('</tr>');
